...
#  Define UDFs
encrypt_DOB_udf = udf(lambda x, row_id: encrypt_value(x, record_id=str(row_id), field="DOB"), StringType())
encrypt_Email_udf = udf(lambda x, row_id: encrypt_value(x, record_id=str(row_id), field="Email"), StringType())

decrypt_DOB_udf = udf(lambda x, row_id: decrypt_value(x, record_id=str(row_id), field="DOB"), StringType())
decrypt_Email_udf = udf(lambda x, row_id: decrypt_value(x, record_id=str(row_id), field="Email"), StringType())

#  Load Original Data
df = spark.read.option("header", True).csv("/FileStore/tables/healthcare_sample_data.csv")
df = df.withColumn("row_id", monotonically_increasing_id())

# Encrypt Columns
df_encrypted = df.withColumn("DOB", encrypt_DOB_udf(col("DOB"), col("row_id"))) \
                 .withColumn("Email", encrypt_Email_udf(col("Email"), col("row_id"))) \
                     .withColumn("Diagnosis", encrypt_Email_udf(col("Diagnosis"), col("row_id")))

df_encrypted.write \
    .format("delta") \
    .mode("overwrite") \
    .option("mergeSchema", "true") \
    .save("/tmp/delta/pii_encrypted1")
...
# Decrypt Columns
df_encrypted = spark.read.format("delta").load("/tmp/delta/pii_encrypted1")

df_decrypted = df_encrypted.withColumn("DOB", decrypt_DOB_udf(col("DOB"), col("row_id"))) \
